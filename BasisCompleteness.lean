...
/-- The inductive multiplicative monoid algebra generated by the four PLEX basis matrices (I, R, A, S)
    and the zero matrix. We omit addition and negation since ISK combinator application corresponds
    only to multiplication, not addition or subtraction. -/
...
/-- The rotation matrix R1 is representable by some term in the substrate. -/
axiom term_matrix_R : ∃ (t : ISKSubtype), term_matrix t = R1

/-- The adjacency matrix A1 is representable by some term in the substrate. -/
axiom term_matrix_A : ∃ (t : ISKSubtype), term_matrix t = A1
...
/--
The Theorem of Expressive Completeness of the ISAR Basis (Proven by Induction):
Any matrix generated in the multiplicative monoid basis algebra of $\{I, R, A, S, \text{zero}\}$ can be expressed

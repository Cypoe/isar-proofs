"""
routines_x86_64_linux_lo — the reducer PROGRAM (asm-as-data) for
ISA=x86_64, ABI=linux (syscall, no imports), order="lo".

SECOND host realization: the reducer algorithm (parser, heap, stepper,
emit_nf, stats, exits, grow_heap) is the routines_x86_64_win64 SHAPE
copied verbatim — same ISA encoder, same lo semantics.  Only the
host-facing layer changes: kernel32 IAT calls become raw syscalls —
read=0/write=1/mmap=9/exit=60, fds 0/1/2, mmap instead of VirtualAlloc:

  mmap(NULL, len, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0)

reporting the IDENTICAL contract: NF tokens on stdout, `steps=N alloc=M`
on stderr (the observation regime must not differ).

Syscall discipline: `syscall` clobbers rcx+r11 (and rax is the return);
args go rdi,rsi,rdx,r10,r8,r9.  Syscall error = negative rax (-errno):
`test rax,rax; jl` is the uniform check (read treats it as EOF, like
win64's ReadFile FALSE; mmap treats it as OOM -> exit4).  The win64
call-site mod-16 convention is kept verbatim — syscalls need no shadow
space, but the internal calls keep their shape so the two modules stay
diff-able routine by routine.
"""
...
# ctx shared by builders: syscall numbers as data (win64's iat operand
# helper analogue — there is no IAT here).
Ctx = Dict[str, object]


def _ctx() -> Ctx:
    return {"sys": {"read": SYS_read, "write": SYS_write,
                    "mmap": SYS_mmap, "exit": SYS_exit}}
...
def r_entry(R: Realization, ctx: Ctx) -> Program:
    ...
    p: Program = [
        ...
        I("mov_r64_imm", "rsi", R.read_buf_bytes),
        I("mov_r32_imm32", "edx", PROT_READ_WRITE),
        I("mov_r32_imm32", "r10d", MAP_PRIV_ANON),
        I("mov_r64_imm", "r8", -1),
        I("xor_r32_r32", "r9d", "r9d"),
        I("mov_r32_imm32", "eax", SYS_mmap),
        I("syscall"),
        I("test_r64_r64", "rax", "rax"), I("jl_rel32", ("l", "exit4")),
...
def r_reduce(R: Realization, ctx: Ctx) -> Program:
    ...
    p += [
        ...
        I("lea_r64_m64", "rsi", ("m", "rax", 0)), I("add_r64_r64", "rsi", "rsi"),
        I("add_r64_imm", "rsi", 16),
        I("xor_r32_r32", "edi", "edi"),
        I("mov_r32_imm32", "edx", PROT_READ_WRITE),
        I("mov_r32_imm32", "r10d", MAP_PRIV_ANON),
        I("mov_r64_imm", "r8", -1),
        ...
        I("mov_r32_imm32", "edi", FD_OUT),
        I("mov_r32_imm32", "eax", SYS_write),
        I("syscall"),
    ]
    return p
...
def r_stats(R: Realization, ctx: Ctx) -> Program:
    ...
    p += [
        ...
        I("lea_r64_rip", "rsi", ("p", "scratch")),
        I("sub_r64_r64", "rdx", "rsi"),
        I("mov_r32_imm32", "edi", FD_ERR),
        I("mov_r32_imm32", "eax", SYS_write),
        I("syscall"),
    ]
    return p
...
def r_exits(R: Realization, ctx: Ctx) -> Program:
    ...
    return [
        I("xor_r32_r32", "edi", "edi"),
        I("mov_r32_imm32", "eax", SYS_exit), I("syscall"),
        LBL("exit2"), I("mov_r32_imm32", "edi", 2),
        I("mov_r32_imm32", "eax", SYS_exit), I("syscall"),
        LBL("exit3"), I("mov_r32_imm32", "edi", 3),
        I("mov_r32_imm32", "eax", SYS_exit), I("syscall"),
        LBL("exit4"), I("mov_r32_imm32", "edi", 4),
        I("mov_r32_imm32", "eax", SYS_exit), I("syscall"),
    ]


def r_grow_heap(R: Realization, ctx: Ctx) -> Program:
    """grow_heap: rbx=bump, rbp=chunk end (chunked mmap).  rsi/rdi are
    saved+restored around the mmap sequence: VirtualAlloc preserved them
    (win64 non-volatile) and the parser keeps cursor/end there — the
    allocator must not clobber what the win64 host call preserved."""
    return [
        LBL("grow_heap"),
        I("push_r64", "rsi"), I("push_r64", "rdi"),
        I("xor_r32_r32", "edi", "edi"),
        I("mov_r64_imm", "rsi", R.chunk_bytes),
        I("mov_r32_imm32", "edx", PROT_READ_WRITE),
        I("mov_r32_imm32", "r10d", MAP_PRIV_ANON),
        I("mov_r64_imm", "r8", -1),
        I("xor_r32_r32", "r9d", "r9d"),
        I("mov_r32_imm32", "eax", SYS_mmap),
        I("syscall"),
        I("pop_r64", "rdi"), I("pop_r64", "rsi"),
        I("test_r64_r64", "rax", "rax"), I("jl_rel32", ("l", "grow_fail")),
        I("mov_r64_r64", "rbx", "rax"),
        I("lea_r64_m64", "rbp", ("m", "rax", R.chunk_bytes)),
        I("ret"),
        LBL("grow_fail"), I("mov_r32_imm32", "edi", 4),
        I("mov_r32_imm32", "eax", SYS_exit), I("syscall"),
    ]
...
X86_64_LINUX_LO = Routines(
    name="x86_64.linux.lo",
    isa="x86_64",
    abi="linux",
    orders=("lo",),
    routines=ROUTINES,
...
# ELF through the spec toolchain chain and run it under WSL — the probes
# are the lo corpus (cd-module shape); NF oracle is
# graph_runtime.reduce_tree_lo, the step oracle is the seed §1 tree
# mirror (graph.lo shares redexes — its count can be lower, exactly the
# distinction seed G3 documents).
# ----------------------------------------------------------------------

def _emit_elf(R: Realization, path: str) -> str:
    """emit(R, tc="x86_64.linux.lo") -> write the ELF — the real chain
    (toolchain.resolve -> isa.assemble -> target.pack) + wsl chmod."""
    import seed
    import toolchain
    import target_elf64
    elf = seed.emit(R, tc=toolchain.by_name("x86_64.linux.lo"))
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, "wb") as f:
        f.write(elf)
    return target_elf64.prepare(path)


def _probes(hreduce, fused: bool):
    """(label, token_text, host_term) — the lo corpus plus a streaming
    probe past the 64KiB read granule (the mmap read path is new code).
    W/C terms are built as host trees and tokenized by hand."""
...
def _run_probes(R: Realization, elf: str, hreduce, bcd) -> bool:
    """Run the emitted ELF under wsl on every probe; NF must equal the
    graph_runtime.reduce_tree_lo oracle (same observation regime as
    win64) and steps the tree mirror's count."""
    import re
    import seed
    import tower
    ...
    for label, text, t in _probes(hreduce, R.fuse_s):
        ...
        out_b, err_b, rc = target_elf64.run_elf(elf, text.encode())
        out, err = (out_b.decode("utf-8", "replace"),
        ...
        if good:
            ...
            if R.fuse_s:
                ...
                nf_ref, _steps_g, _alloc_g = graph_runtime.reduce_tree_lo(
                    base, fuel=1_000_000)
                expect_nf = nf_ref
                got = tower.quote_surface(got)
                _nf_seed, steps_ref = seed.reduce(
                    seed.bc_compile(text), fuel=1_000_000)
            good = (got == expect_nf) and (steps_native == steps_ref)
            if not good:
                detail = (f" nf={got!r} ref={expect_nf!r} "
                          f"steps={steps_native} mirror={steps_ref}")
                print(f"  FAIL {label}: rc={rc} out={out.strip()!r} "
                      f"err={err.strip()!r}{detail}")
                ok = False
            else:
                print(f"  ok {label}: nf={out.strip()[:80]!r} "
                      f"steps={steps_native}")
        else:
            print(f"  FAIL {label}: rc={rc} out={out.strip()!r} "
                  f"err={err.strip()!r}")
            ok = False
...
def main() -> int:
    ...
    for tag, R in (("default", Realization(abi="linux")),
        ...
        if has_ds != (not R.fuse_s) or has_s != R.fuse_s:
            ...
            ok = False
    # honest refusal: this module realizes order=lo on abi=linux only
    ...
    for tag, R in (("default", Realization(abi="linux")),
                   ("fuse_s", Realization(abi="linux", fuse_s=True))):
        elf = _emit_elf(R, os.path.join(
            _seed_dir, "build",
            "reducer_linux.elf" if tag == "default"
            else "reducer_linux_fuse.elf"))
        print(f" build {tag}: {elf} ({os.path.getsize(elf)}B)")
        ok = _run_probes(R, elf, hreduce, bcd) and ok
    print(f"{'OK' if ok else 'FAIL'} routines_x86_64_linux_lo")
